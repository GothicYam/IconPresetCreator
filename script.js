...
// Function to get the selected version (1.5.6 or 1.5.3)
...
document.getElementById('submit-button').addEventListener('click', () => {
    ...
    let sortedOffsets = selectedOffsets
        .map(offset => parseInt(offset, 16)) // Convert hex to decimal
        .sort((a, b) => a - b); // Sort in ascending order

    // Apply 0x48 subtraction for "1.5.3" version if selected
    if (getSelectedVersion() === "1.5.3") {
        sortedOffsets = sortedOffsets.map(offset => offset - 0x48);
    }

    // Convert back to hex and uppercase
    sortedOffsets = sortedOffsets.map(offset => offset.toString(16).toUpperCase());
    ...
    // Generate the string output in JSON-like format
    const presetTitle = `Preset - ${document.getElementById('preset-title').value || "Untitled"}`;
    ...
    // Append details in JSON format
    stringOutput += `\t"PatchTitle": "${presetTitle}",\n`;
    stringOutput += `\t"PatchDescription": "${presetDescription}",\n`;
    stringOutput += `\t"PatchID": "${patchID}",\n`;
    stringOutput += `\t"PatchVersion": 1,\n`;
    stringOutput += `\t"Targets": [\n`;
    stringOutput += `\t\t{\n`;
    stringOutput += `\t\t\t"File": "DataPC_patch_01.forge",\n`;
    stringOutput += `\t\t\t"FileID": 69568652640,\n`;
    stringOutput += `\t\t\t"Edits": [\n`;
    stringOutput += `\t\t\t\t{\n`;
    stringOutput += `\t\t\t\t\t"OffsetHex": "${startingHex}",\n`;
    stringOutput += `\t\t\t\t\t"RunCount": ${skips.length + 1},\n`;
    stringOutput += `\t\t\t\t\t"Skip": [${skips.map(skip => skip.toString()).join(', ')}],\n`;
    stringOutput += `\t\t\t\t\t"Data": "${visibilityType}"\n`;
    stringOutput += `\t\t\t\t}\n`;
    stringOutput += `\t\t\t]\n`;
    stringOutput += `\t\t}\n`;
    stringOutput += `\t]\n`;
    ...
    // Display the JSON-like string output in the modal
    document.getElementById('json-output').textContent = stringOutput;
